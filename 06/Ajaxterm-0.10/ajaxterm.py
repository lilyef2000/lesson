...
class AjaxTerm:
	def __init__(self,cmd=None,index_file='ajaxterm.html'):
		self.tempVar=[]  #set the list to store the input before write to file
		self.files={}
	...
	def __call__(self, environ, start_response):
		...
		if req.PATH_INFO.endswith('/u'):
			s=req.REQUEST["s"] #session Id
			k=req.REQUEST["k"] #command key
			c=req.REQUEST["c"]
			w=req.REQUEST.int("w")
			h=req.REQUEST.int("h")
			...
			if k:
				#Todo by Liuyulei  ==========
				try:
					if k=='\x7f' and len(self.tempVar)>0:
						self.tempVar=self.tempVar[0:-1]
					elif k=='\r':
						fp=open('test.txt','a')
						record = "\r\n%s\t%s" %(ctime(),''.join(self.tempVar))
						#fp.write('\r\n'+(''.join(self.tempVar)))
						fp.write(record)
						fp.close()
						self.tempVar=[]
